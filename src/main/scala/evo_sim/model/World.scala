...
object World {
  ...
  def worldEnvironmentUpdated(world: World): EnvironmentParameters = {
    val luminosityUpdated: ((Int, Float)) => Int = MemoHelper.memoize({
      case (luminosity, x) =>
        luminosity + zeroPhasedZeroYTranslatedSinusoidalSin(Constants.LUMINOSITY_AMPLITUDE)(x)
    })

    val temperatureUpdated: ((Int, Float)) => Int = MemoHelper.memoize({
      case (temperature, x) =>
        temperature + zeroPhasedZeroYTranslatedSinusoidalSin(Constants.TEMPERATURE_AMPLITUDE)(x)
    })

    val x = world.currentIteration.toFloat / Constants.ITERATIONS_PER_DAY.toFloat
    EnvironmentParameters(luminosityUpdated(world.luminosity, x), temperatureUpdated(world.temperature, x))
  }
